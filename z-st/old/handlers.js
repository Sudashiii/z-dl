import { corsHeaders, mimeTypes } from './utils.js'
...
const bucket = r2.bucket('bookodav');
...
export async function handleDeleteFile(request, env, ctx) {
    ...
    try {
        await bucket.deleteObject(filePath);
...
export async function handleFileList(request, env, ctx) {
    ...
    const objects = await bucket.listObjects(100, prefix);
    return
    console.log(objects);
